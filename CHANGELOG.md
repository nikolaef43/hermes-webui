...
- `start.sh` now loads non-readonly `.env` keys after filtering Docker-style `UID`/`GID` entries on macOS/bash combinations where `source <(grep ...)` produced an empty sourced stream.
